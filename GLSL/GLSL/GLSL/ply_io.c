...
void ascii_get_element(PlyFile *plyfile, char *elem_ptr)
{
  ...
  elem = plyfile->which_elem;
  char *other_data = (char *)myalloc(elem->other_size); // CHANGED!!
  ...
  if (elem->other_offset != NO_OTHER_PROPS) {
    ...
    //other_data = (char *) myalloc (elem->other_size); // CHANGED!!
...
void add_property (PlyFile *plyfile, char **words, int nwords)
{
  ...
  if (equal_strings (words[1], "list")) {          /* list */
	...
	//Additional
	if (equal_strings("red", words[2]) || equal_strings("r", words[2]))
		prop->name = _strdup("r");
	else if (equal_strings("green", words[2]) || equal_strings("g", words[2]))
		prop->name = _strdup("g");
	else if (equal_strings("blue", words[2]) || equal_strings("b", words[2]))
		prop->name = _strdup("b");
	else
    prop->name = _strdup (words[2]);
...
void *get_new_props_ply(PlyFile *ply)
{
  ...
  random_pick = (int) floor (rules->nprops * rand_s());
