...
void Window::View(Scene* s, int width, int height, int cplx){
	int drawMenu;
	int cullMenu;
//	int curveFitMenu;
	int argc = 1;
...
//void Window::WriteString(int x, int y, void* font, bool bottom, bool left, const char* format, ...)
//{
...
//	glGetIntegerv(GL_VIEWPORT, vp);
//
//	if (!bottom) y = vp[3] - y - fontHeight(font);
//	if (!left)   x = vp[2] - x - glutBitmapLength(font, (unsigned char*)str);
//
//	glPushMatrix();
//	glLoadIdentity();
//	glMatrixMode(GL_PROJECTION);
